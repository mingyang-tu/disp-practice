...
def harris_corner(img: NDArray[np.float64], window: NDArray[np.float64], k: float, threshold: float) -> tuple[list[list[int]], NDArray[np.float64]]:
    ...
    mat_A = signal.convolve2d(np.square(mat_Ix), window, boundary='symm', mode='same')
    mat_B = signal.convolve2d(np.square(mat_Iy), window, boundary='symm', mode='same')
    mat_C = signal.convolve2d(mat_Ix * mat_Iy, window, boundary='symm', mode='same')
